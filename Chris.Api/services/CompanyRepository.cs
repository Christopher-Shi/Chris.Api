...
namespace Chris.Api.services
{
    ...
    {
        private readonly ChrisDbContext _context;
        private readonly IPropertyMappingService _propertyMappingService;

        public CompanyRepository(ChrisDbContext context, IPropertyMappingService propertyMappingService)
        {
        ...
        public async Task<IEnumerable<Employee>> GetEmployeesAsync(Guid companyId, EmployeeDtoParameters parameters)
        {
            ...
            if (!string.IsNullOrWhiteSpace(parameters.Q))
            {
                ...
                items = items.Where(x => x.EmployeeNo.ToLower().Contains(parameters.Q.ToLower())
                                         || x.FirstName.ToLower().Contains(parameters.Q.ToLower())
                                         || x.LastName.ToLower().ToLower().Contains(parameters.Q.ToLower()));
            }
